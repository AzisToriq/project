...
<head>
    ...
    <style>
        :root {
            --primary-blue: #2a6ebb;
            --primary-blue-dark: #1d4f8f;
            --primary-green: #2ecc71;
            --primary-green-dark: #27ae60;
            --gradient-start: #2a6ebb;
            --gradient-end: #2ecc71;
            --text-dark: #2c3e50;
            --text-light: #7f8c8d;
            --bg-light: #f8fafc;
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* --- CSS SAMA PERSIS DENGAN KODE ASLIMU --- */
        ...
        .custom-input:focus { outline: none; border-color: var(--primary-blue); background-color: white; box-shadow: 0 0 0 3px rgba(42, 110, 187, 0.1); }
        .custom-input.error { border-color: var(--error-color); }
        .custom-input.success { border-color: var(--success-color); }

        /* Tambahan CSS: Sembunyikan clear button secara default */
        ...
        #toggleEmailClear { display: none; } /* Default hidden */
        ...
        .divider { display: flex; align-items: center; text-align: center; margin: 25px 0; color: var(--text-light); font-size: 14px; }
        .divider::before, .divider::after { content: ''; flex: 1; border-bottom: 1px solid #e9ecef; }
        .divider span { padding: 0 15px; }

        .alt-login { display: flex; justify-content: center; gap: 15px; }
        .alt-login-btn { display: flex; align-items: center; justify-content: center; width: 50px; height: 50px; border-radius: 50%; background-color: #f8f9fa; border: 1px solid #e9ecef; color: var(--text-dark); font-size: 20px; cursor: pointer; transition: all 0.2s ease; }
        .alt-login-btn:hover { background-color: #f1f3f5; transform: translateY(-3px); box-shadow: 0 5px 10px rgba(0, 0, 0, 0.05); }
        ...
        @media (max-width: 900px) { .main-container { flex-direction: column; width: 95%; max-width: 500px; min-height: auto; } .info-panel { padding: 30px; } .login-panel { padding: 30px; } .info-title { font-size: 28px; } .login-title { font-size: 26px; } }
        @media (max-width: 480px) { .info-panel, .login-panel { padding: 25px; } .form-options { flex-direction: column; align-items: flex-start; gap: 15px; } .info-title { font-size: 24px; } .login-title { font-size: 22px; } }
...
<body>
    ...
    <div class="main-container">
        <div class="info-panel">
            <div class="logo-container">
                ...
                <div class="logo-text">
                    <h1>SIAKAD PRO</h1>
                    <p>Sistem Informasi Akademik Profesional</p>
                </div>
            </div>

            <div class="info-content">
                <h2 class="info-title">Selamat Datang di Platform Pendidikan Digital</h2>
                <p class="info-description">
                    Sistem Informasi Akademik yang terintegrasi untuk mengelola data siswa, kurikulum, penilaian, dan aktivitas sekolah secara efektif dan efisien.
                </p>

                <div class="features">
                    <div class="feature"><div class="feature-icon"><i class="fas fa-check"></i></div><div class="feature-text">Manajemen Data Siswa & Guru Terpusat</div></div>
                    <div class="feature"><div class="feature-icon"><i class="fas fa-check"></i></div><div class="feature-text">Penilaian Akademik Otomatis</div></div>
                    <div class="feature"><div class="feature-icon"><i class="fas fa-check"></i></div><div class="feature-text">Laporan Real-time & Analytics</div></div>
                    <div class="feature"><div class="feature-icon"><i class="fas fa-check"></i></div><div class="feature-text">Keamanan Data Tingkat Tinggi</div></div>
                </div>
            </div>

            <div class="info-footer">
                <p>© 2025 SIAKAD PRO. Hak Cipta Dilindungi.</p>
            </div>
        </div>
        ...
        <div class="login-panel">
            <div class="login-header">
                ...
                <p class="login-subtitle">Gunakan kredensial Anda untuk mengakses dashboard</p>
            </div>
            ...
            @if ($errors->any())
                <div class="auth-session-status error">
                    ...
                    <span>Kredensial yang Anda masukkan salah.</span>
                </div>
            @endif
            ...
            <div class="form-container">
                <form method="POST" action="{{ route('login') }}" class="login-form" id="loginForm" novalidate>
                    ...
                    <div class="input-group">
                        <label for="email" class="input-label">Email Sekolah</label>
                        <div class="input-container">
                            <div class="input-icon"><i class="fas fa-envelope"></i></div>
                            <input id="email" type="email" name="email" value="{{ old('email') }}"
                                   class="custom-input @error('email') error @enderror"
                                   placeholder="contoh@sekolah.sch.id" required autofocus aria-label="Alamat Email">

                            <div class="password-toggle" id="toggleEmailClear" role="button" tabindex="0" aria-label="Hapus Email">
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-container">
                            ...
                            <input id="password" type="password" name="password"
                                   class="custom-input @error('password') error @enderror"
                                   placeholder="Masukkan kata sandi Anda" required aria-label="Kata Sandi">

                            <div class="password-toggle" id="togglePassword" role="button" tabindex="0" aria-label="Tampilkan Kata Sandi">
                    ...
                    <button type="submit" class="btn-primary pulse" id="submitBtn">
                    ...
                    <div class="alt-login">
                        <button type="button" class="alt-login-btn" title="Masuk dengan Google"><i class="fab fa-google"></i></button>
                        <button type="button" class="alt-login-btn" title="Masuk dengan Microsoft"><i class="fab fa-microsoft"></i></button>
                        <button type="button" class="alt-login-btn" title="Masuk dengan Biometrik"><i class="fas fa-fingerprint"></i></button>
                    </div>
                </form>
            </div>

            <div class="login-footer">
                <p>Belum memiliki akun? <a href="#">Hubungi Administrator Sekolah</a></p>
                <p>© 2025 SIAKAD PRO. Secure Login dengan enkripsi AES-256.</p>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- 1. Toggle Visibility Email Clear Button ---
            ...
            // Jalankan saat load dan saat mengetik
            ...
            // A11y: Support enter key on X button
            clearEmailBtn.addEventListener('keydown', function(e) {
            ...
            // --- 3. Form Submission Handling (Clean Logic) ---
            loginForm.addEventListener('submit', function(e) {
                // Gunakan validasi HTML5 bawaan browser dulu
                if (!this.checkValidity()) {
                    // Biarkan browser menampilkan bubble error bawaan
                    return;
                }

                // Jika valid, tampilkan loading state
                // Kita TIDAK preventDefault disini agar form benar-benar submit ke Laravel

                // Ubah tampilan tombol
                btnText.innerText = 'MEMPROSES...';
                btnIcon.className = 'fas fa-spinner fa-spin';
                submitBtn.style.opacity = '0.8';
                submitBtn.style.pointerEvents = 'none'; // Mencegah double submit
            });
